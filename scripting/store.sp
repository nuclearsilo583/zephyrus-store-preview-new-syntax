...
Client g_eClients[MAXPLAYERS+1];
...
public int Native_DisplayConfirmMenu(Handle plugin, int numParams)
{
	...
	char m_szCallback[32];
	char m_szData[11];
	IntToString(view_as<int>(pack), m_szCallback, sizeof(m_szCallback));
	IntToString(GetNativeCell(4), m_szData, sizeof(m_szData));

	//delete pack;
	Menu menu = new Menu(MenuHandler_Confirm);
	menu.SetTitle(title);

	Format(title, sizeof(title), "%t", "Confirm_Yes");
	menu.AddItem(m_szCallback, title, ITEMDRAW_DEFAULT);

	Format(title, sizeof(title), "%t", "Confirm_No");
	menu.AddItem(m_szData, title, ITEMDRAW_DEFAULT);
	//Zephyrus magic
...
public int Native_GetHandler(Handle plugin,int numParams)
{
	...
	SetNativeArray(2, g_eTypeHandlers[iIndex], sizeof(Type_Handler));
...
public int Native_GetClientItem(Handle plugin,int numParams)
{
	...
	//SetNativeArray(2, g_eItems[itemID], sizeof(Store_Item));
	////SetNativeArray(2, view_as<int>(g_eItems[GetNativeCell(1)][GetNativeCell(2)]), sizeof(g_eItems[][])); 
	SetNativeArray(3, g_eClientItems[client][uid], sizeof(Client_Item)); 
...
public Action Command_Gift(int client,int params)
{
	...
	int m_iClients = ProcessTargetString(m_szTmp, 0, m_iTargets, 1, 0, STRING(m_szTmp), m_bTmp);
	if(m_iClients>2)
	{
...
public int MenuHandler_Store(Handle menu, MenuAction action,int client,int param2)
{
	...
	//(g_aItems[iIndex].bPreview && !HasClientItem(target, iIndex) && g_aItems[iIndex].iPrice != -1 && g_aItems[iIndex].iPlans == 0)
				
				//(g_eClients[target].iCredits>=g_eItems[m_iId].iPrice
				//|| g_eItems[m_iId].iPlans>0 && g_eClients[target].iCredits>=Store_GetLowestPrice(m_iId))
...
public DisplayPlanMenu(client, itemid)
{
	...
	if (g_eItems[itemid].bPreview)
	{	
		AddMenuItemEx(m_hMenu, ITEMDRAW_DEFAULT, "preview_item", "%t", "Preview Item");
	}
...
/*
public MenuHandler_Plan(Handle menu, MenuAction action,int client,int param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		if (strcmp(sId, "preview") == 0)
		{
			if (g_iSpam[client] > GetTime())
			{
				CPrintToChat(client, "%s%t", " {yellow}[Store] {lime}", "Spam Cooldown", g_iSpam[client] - GetTime());
				DisplayPlanMenu(client, itemid);
				return;
			}
			...
			if (!IsPlayerAlive(client))
			{
				CPrintToChat(client, "%s%t", " {yellow}[Store] {lime}", "Must be Alive");
				DisplayPlanMenu(client, itemid);
				return;
			}
...
{
	for (int i = 0; i < g_eItems[client]; i++)
	{
		if (g_eClientItems[client][i][UNIQUE_ID] == itemid && g_eClientItems[client][i][DELETED] == 0)
			return i;
	}
