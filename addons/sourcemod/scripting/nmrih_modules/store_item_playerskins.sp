...
{
	name = "Store - Player Skin Module [NMRIH]",
...
{
	int m_iData = Store_GetDataIndex(id);
	if (Store_IsClientLoaded(client) && g_eCvars[g_bSkinEnable].aCache == 1 && IsValidClient(client, true))
	{
		CPrintToChat(client, " %s%t", g_sChatPrefix, "PlayerSkins Settings Changed");
	}
	else CPrintToChat(client, "%s%t", g_sChatPrefix, "Player Skin module disabled");
	
	return view_as<int>(g_ePlayerSkins[m_iData].iTeam)-2;
}
